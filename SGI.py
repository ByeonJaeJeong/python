...
class MainFrame(tk.Tk):
    ...
    def __init__(self):
        ...
        self.pathEntry.bind("<Button-1>", self.path_click)
        #검색하는 프레임
        ...
        # 중지 버튼
        self.stopButton = tk.Button(actionFrame, text="정지", width="8", state=tk.DISABLED)
        self.stopButton.pack(side="right", padx=3)
        # 시작 버튼
        self.startButton = tk.Button(actionFrame, text="다운로드", width="8")
        self.startButton.pack(side="right", padx=3, anchor="s")
    #저장위치 설정 클릭시 event 처리
    def path_click(self,event):
